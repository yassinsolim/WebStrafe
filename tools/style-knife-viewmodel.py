"""Restyle knife-viewmodel arms as dark gloves/sleeves without rebaking its rig.

Usage:
  blender --background --python tools/style-knife-viewmodel.py -- SOURCE.glb OUTPUT.glb
"""
...
def write_styled_texture(image: bpy.types.Image, output: Path) -> int:
    ...
    for index in range(0, len(pixels), 4):
        ...
        pixels[index] = DARK_GLOVE_BASE[0] * detail
        pixels[index + 1] = DARK_GLOVE_BASE[1] * detail
        pixels[index + 2] = DARK_GLOVE_BASE[2] * detail
        changed += 1
...
def main() -> None:
    source_text, output_text = sys.argv[sys.argv.index("--") + 1 :]
    ...
    with tempfile.TemporaryDirectory(prefix="webstrafe-knife-style-") as temp:
        texture_path = Path(temp) / "arms-black-sleeves.png"
        changed = write_styled_texture(
            find_base_color_image(material),
            texture_path,
        )
        replace_base_color_image(source, output, texture_path.read_bytes())
    print(f"Styled {changed} knife-arm pixels into {output}")
